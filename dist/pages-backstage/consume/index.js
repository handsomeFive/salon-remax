...
var _useState7=Object(react["useState"])(),_useState8=_slicedToArray(_useState7,2),custom=_useState8[0],setCustom=_useState8[1];// 自定义金额
...
var _useState11=Object(react["useState"])([]),_useState12=_slicedToArray(_useState11,2),product=_useState12[0],setProduct=_useState12[1];var _useState13=Object(react["useState"])(false),_useState14=_slicedToArray(_useState13,2),loading=_useState14[0],setLoading=_useState14[1];var handleSelectProduct=Object(react["useCallback"])(function(event){setCurrent(event.detail.value);},[]);var handleChangeInput=Object(react["useCallback"])(function(type,event){var value=event.detail.value;var handle={number:setNumber,spend:setSpend,presenter:setPresenter,custom:setCustom}[type];handle(value);},[]);var handleCreate=Object(react["useCallback"])(function(){if(!number){ling.current.error('请输入手机号');return;}else if(type==='1'&&!spend){ling.current.error('请输入充值金额');return;}else if(type==='1'&&Number(spend)<0){ling.current.error('充值金额不能小于0');return;}else if(type==='2'&&!current&&!custom){ling.current.error('请选择或输入价格');return;}var path=type==='1'?'/order/recharge':'/order/consume';var data={phoneNumber:number};if(type==='1'){// 充值
